...
MAXX = 3
MUESTREO = 1. # segundos
archivo = path.join(getcwd(), 'setup.dat')


#  Esta función decide el nombre del archivo .csv
def archivoPorFecha(tiempo, archivo):
  a = archivo.split('.')
  date_tiempo = date.fromtimestamp(tiempo)  # convierte el timestamp tiempo (q
                                            # es un float en unidad de segundos)
                                            # a un objeto `date` que es para fechas.
  date_string = date_tiempo.strftime('%Y-%m-%d')  # devuelve un texto con formato
                                                  # YYY-MM-DD a partir del objeto `date`
  return "{}.{}".format(date_string, a[1])  # completa la cadena de texto "{}.{}"
                                            # con los valores q se le pasan en el
                                            # método .format()
